...
class GetVideoImagesDownloadUrlUseCaseTest {
	...
	private final GetVideoImagesDownloadUrlUseCase useCase = new GetVideoImagesDownloadUrlUseCase(videoGateway);
	...
	void shouldReturnDownloadUrlWhenInputsAreValid() {
		...
		when(videoGateway.getVideoImagesDownloadUrl(userId, requestId, videoName, 30L)).thenReturn(expectedUrl);
	...
	void shouldThrowExceptionWhenUserIdIsNull() {
		// Arrange & Act
		var ex = assertThrows(VideoException.class, () -> useCase.getVideoImagesDownloadUrl(null, "req", "video"));

		// Assert
		assertEquals("userId não pode ser nulo ou vazio para pesquisa de URL de download das imagens", ex.getMessage());
	}

	@Test
	void shouldThrowExceptionWhenRequestIdIsBlank() {
		// Arrange & Act
		var ex = assertThrows(VideoException.class, () -> useCase.getVideoImagesDownloadUrl("user", " ", "video"));

		// Assert
		assertEquals("requestId não pode ser nulo ou vazio para pesquisa de URL de download das imagens",
				ex.getMessage());
	}
	...
	void shouldThrowExceptionWhenVideoNameIsNull() {
		// Arrange & Act
		var ex = assertThrows(VideoException.class, () -> useCase.getVideoImagesDownloadUrl("user", "req", null));

		// Assert
		assertEquals("videoName não pode ser nulo ou vazio para pesquisa de URL de download das imagens",
				ex.getMessage());
	}
	...
	void shouldThrowExceptionWhenDownloadUrlIsNotFound() {
		...
		when(videoGateway.getVideoImagesDownloadUrl(userId, requestId, videoName, 30L)).thenReturn(" ");
