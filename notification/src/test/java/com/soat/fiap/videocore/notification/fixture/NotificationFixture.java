...
/**
 * Fixture para criação de instâncias válidas de {@link Notification} para uso
 * em testes unitários.
 */
public final class NotificationFixture {
	...
	/**
	 * Cria uma instância válida de {@link Recipient} contendo um e-mail
	 * estruturado.
	 *
	...
	/**
	 * Cria uma instância completa e válida de {@link Notification} utilizando os
	 * métodos auxiliares desta fixture.
	 *
	 * @return uma entidade de notificação pronta para uso em testes.
	 */
	public static Notification validNotification() {
		return new Notification(validRecipientName(), validSubject(), validRecipient(), validMessage());
	}
	...
	public static UserDTO user() {
		return new UserDTO("user-id", "João Silva", "joao@email.com");
	}
	...
	/**
	 * Cria um {@link ProcessVideoStatusUpdateInput} representando o início do
	 * processamento do vídeo (percentual igual a 0).
	 *
	 * @return payload de início de processamento.
	 */
	public static ProcessVideoStatusUpdateInput notificationInputStarted() {
		return new ProcessVideoStatusUpdateInput("video.mp4", UUID.randomUUID().toString(),
				UUID.randomUUID().toString(), UUID.randomUUID().toString(), 0L, 1L, 0D, Instant.now(), false);
	}

	/**
	 * Cria um {@link ProcessVideoStatusUpdateInput} representando a finalização do
	 * processamento do vídeo (percentual igual a 100).
	 *
	 * @return payload de finalização de processamento.
	 */
	public static ProcessVideoStatusUpdateInput notificationInputFinished() {
		return new ProcessVideoStatusUpdateInput("video.mp4", UUID.randomUUID().toString(),
				UUID.randomUUID().toString(), UUID.randomUUID().toString(), 10L, 1L, 100D, Instant.now(), false);
	}

	/**
	 * Cria um {@link ProcessVideoErrorInput} representando um evento de erro
	 * durante o processamento do vídeo.
	 *
	 * @return payload de processamento com erro.
	 */
	public static ProcessVideoErrorInput notificationErrorInput() {
		return new ProcessVideoErrorInput("video.mp4", UUID.randomUUID().toString(), UUID.randomUUID().toString(),
				UUID.randomUUID().toString(), 5L, 50D, Instant.now());
	}

	/**
	 * Cria um {@link ProcessVideoStatusUpdateInput} representando um evento de erro
	 * durante o processamento do vídeo.
	 *
	 * @return payload de processamento com erro.
	 */
	public static ProcessVideoStatusUpdateInput notificationInputWithError() {
		return new ProcessVideoStatusUpdateInput("video.mp4", UUID.randomUUID().toString(),
				UUID.randomUUID().toString(), UUID.randomUUID().toString(), 5, 5, 50D, Instant.now(), true);
	}
