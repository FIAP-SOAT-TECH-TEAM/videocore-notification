 ...
 * {@link CreateEmailNotificationErrorProcessUseCase}.
 */
class CreateEmailNotificationErrorProcessUseCaseTest {

	private final CreateEmailNotificationErrorProcessUseCase useCase = new CreateEmailNotificationErrorProcessUseCase();

	@Test
	void shouldCreateNotificationWhenInputIsValid() {
		// Arrange
		var user = new UserDTO("1", "João", "joao@email.com");
		var input = new ProcessVideoErrorInput("video.mp4", UUID.randomUUID().toString(), UUID.randomUUID().toString(),
				UUID.randomUUID().toString(), 0, 0.0, Instant.now());
	...
	void shouldThrowExceptionWhenUserIsNull() {
		// Arrange
		var input = new ProcessVideoErrorInput("video.mp4", null, UUID.randomUUID().toString(),
				UUID.randomUUID().toString(), 1, 20.0, Instant.now());
	...
	void shouldThrowExceptionWhenEmailIsInvalid() {
		...
		var input = new ProcessVideoErrorInput("video.mp4", UUID.randomUUID().toString(), UUID.randomUUID().toString(),
				UUID.randomUUID().toString(), 5, 50.00, Instant.now());

		// Act
		var ex = assertThrows(UserException.class, () -> useCase.createEmailNotificationErrorProcess(user, input));
