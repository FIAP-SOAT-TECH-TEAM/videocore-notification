 ...
 * Valida todos os fluxos possíveis de notificação durante o processamento de
 * vídeo.
 */
...
class ProcessVideoStatusUpdateControllerTest {
	...
	void shouldSendFinishedProcessNotificationWhenPercentIsHundred() {
		...
		when(createEmailNotificationFinishedProcessUseCase.createEmailNotificationFinishedProcess(user, input,
				"http://download")).thenReturn(notification);
